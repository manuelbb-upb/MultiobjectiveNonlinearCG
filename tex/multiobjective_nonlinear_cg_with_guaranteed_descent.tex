...
\begin{proof}
	...
	% $$
	% Thus, for any vector $\vd \in \RRin$,
	% \begin{align*}
	% \mul{\vd}{\sdK}
	% %&
	% =
	% -\mul{\vd}{\transp{\jacf(\vxK)}\ve{w}^*}
	% %\\
	% %&
	% \geq
	% \min_{\ve w \in C}
	% -\mul{\vd}{\jacf(\vxK)\ve w}
	% =
	% -
	% \max_{\ve w \in C}
	% \mul{\vd}{\transp{\jacf(\vxK)\ve w}}
	...
	% Hence, by reversing the signs in this inequality, 
	% \begin{equation}
	% -\mul{\sdK}{\vdKp}
	% \leq
	% \funcXK(\vdKp)
	% \label{eqn:customizeGarrons}
	% \end{equation}
	% and
	% especially, we get
	% $$
...
\begin{proposition}
	%If\cref{ass:funcs_differentiable,ass:sublevelset_bounded} hold,
	%then 
	The directions in~\eqref{eqn:frMO2} have the 
	sufficient decrease property~\eqref{eqn:suff_dec}.
	%with some constant $\suffDecConst > 0$.
	% Moreover, it holds that 
	% $$
	% \mul{\sdK}{\vdK} \leq \norm{\sdK}^2 \qquad \forall \kk\in\mathbb N_0.
	% $$
\end{proposition}
...
\begin{proof}
	...
	%we have plugged in the 
	%definition~\eqref{eqn:characteriserAbout} and used
\end{proof}

% \begin{remark*}
% 	Instead of having $\beta\kK$ depend on $\ve w^*$ in \eqref{eqn:characteriserAbout},
% 	we can also use $\beta\kK = \norm{\sdK}^2/\norm{\sdKp}^2$ again.
% 	The proof works just the same.
% 	After plugging in the coefficient definitions and 
% 	using the MaxiMin property, we only have to realize that
% 	\begin{align*}
% 	\mul{\ve v^*}{\jacf(\vxK)\vdKp}
% 	\mul{\ve{v}^*}{\jacf(\vxK)\sdK}
% 	-
...
% 	\leq 0,
% 	\end{align*}
% 	to obtain the same final inequalities.
% \end{remark*}
...
\begin{proof}
	\Cref{ass:sublevelset_bounded} implies
...
\todo{References: von Neumann and Generalized Rock Paper Scissors}
...
\todo{
	The proof below can be easily adapted to include the factors 
	$\alpha_\beta, \alpha_\theta \in [0,1]$.
}
Assume $\kk \geq 1$.
